...
function sortTable() {
    ...
    if (sortedByColumn === 4) sortRowsBySalary(rows, asc);
    else sortRows(rows, asc, sortedByColumn);
...
function addEmployee(event) {
    ...
    tableData.columns.forEach((_column, index)=>{
        ...
        if (index === 4) input = formatCurrency(input);
        if (!input) hasError = true;
        if (index === 3 && (input < 18 || input > 90) || index === 0 && input.length < 4 || index === 2 && input.length === 0) hasError = true;
        cell.textContent = input || ""; // Ensure empty string if no data
        newRow.appendChild(cell);
    });
    const requiredFields = form.querySelectorAll("[required]");
    requiredFields.forEach((field)=>{
        if (!field.value) hasError = true;
    });
    if (hasError) {
...
function enableCellEditing() {
    ...
    tableData.tableBody.addEventListener("dblclick", (event)=>{
        ...
        if (cell.tagName !== "TD" || cell.querySelector("input")) return;
        if (currentInput) saveCell(currentInput);
        ...
        input.addEventListener("keypress", (e)=>{
            if (e.key === "Enter") saveCell(input, initialValue);
        });
        currentInput = input;
    });
